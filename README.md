...
- `config` - configuration object *(Optional)*
  ...
  - `[measurementOverride]` - This is a function which can be provided if you would like to override which measurement to use based on other data than the event name. The function is called with the whole `event` object. Changing the measurement does _not_ affect the processing of its data. If this is provided, `prefix` and `prefixDelimiter` are both ignored.
    - Example: If you need more granular measurements based on your tags, you can have this change any events with a tag of, say, `metrics` to instead route to a measurement of `metrics`.
  - `[valueTransformer]` - This function gets called, if present, after the event data has been processed and squashed. It is passed the squashed data, the event, and the `Formatters`. If present, this function must return an object representing the field data. The purpose of this function is to make any last minute transformations or formatting changes to the data before it gets sent. A good example of use is if you have a field which is sometimes a string and sometimes a float. This will allow you to choose one or otherwise delete the field if none apply (so that your log entry gets logged despite the type mismatch).
    - Example: In logging data for different downstream calls, sometimes the `status` field will be a integer code (for http calls), and sometimes it will be just "success" or "failure". You can't log both integer and string to influx, so you would use this to transform any which come in as integers to instead be strings.
    - Example 2: In logging float number values, sometimes your data may present in a way that the float looks like an integer to javascript. This would allow you to reformat it to always process as a float. Conversely, if you did not need float level granularity you could always parse it to an integer instead.
```json
{
  "event": "log",
