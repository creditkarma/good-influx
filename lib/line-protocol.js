...
internals.values = {
    ...
    log: (event, config) => {
        ...
        const customLogFormatters = config.customLogFormatters;
        if (customLogFormatters) {
            const tag = Helpers.firstCommonItem(Object.keys(customLogFormatters), event.tags);
            if (tag) {
                const customLogFormatter = customLogFormatters[tag];
                if (!Helpers.isFunction(customLogFormatter)) {
                    throw new Error('customLogFormatter should be a function');
                }
                const customlogField = customLogFormatter(event.data);
                return Object.keys(customlogField).reduce((agg, key) => {
                    ...
                    if (!isNaN(val)) {
                        ...
                        agg[Formatters.TagKV(key)] = Formatters.String(val);
                    }
                    return agg;
                }, {});
            }
...
module.exports.format = (event, config) => {
    ...
    // Field set
    let eventValues = getEventValues(event, config);

    if (typeof config.valueTransformer === 'function') {
        eventValues = config.valueTransformer(eventValues, event, Formatters)
    }
    ...
    let finalEventName
    if (typeof config.measurementDeterminer === 'function') {
        finalEventName = config.measurementDeterminer(event)
    } else {
        const prefix = Helpers.measurementPrefix(config);
        finalEventName = Formatters.Measurement(`${prefix}${eventName}`);
    }
