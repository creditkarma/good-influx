...
internals.values = {
    ...
    log: (event) => {
        ...
        if (event.data.stats) {
            const stats = {};
            const keys = Object.keys(event.data.stats);
            for (let i = 0; i < keys.length; ++i) {
                const val = event.data.stats[keys[i]];
                if (!isNaN(val)) {
                    stats[Formatters.TagKV(keys[i])] = Formatters.Int(val);
                }
                else if (!isNaN(parseFloat(val))) {
                    stats[Formatters.TagKV(keys[i])] = Formatters.Int(parseFloat(val));
                }
            }
            return stats;
        }
